...
tape( 'the function returns `true` if provided a valid array length', function test( t ) {
	t.strictEqual( isArrayLength( 0 ), true, 'returns expected value' );
	t.strictEqual( isArrayLength( 10 ), true, 'returns expected value' );
	t.strictEqual( isArrayLength( MAX_ARRAY_LENGTH ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `false` if not provided a valid array length', function test( t ) {
	t.strictEqual( isArrayLength( MAX_ARRAY_LENGTH+1 ), false, 'returns expected value' );
	t.strictEqual( isArrayLength( -1 ), false, 'returns expected value' );
	t.strictEqual( isArrayLength( 3.14 ), false, 'returns expected value' );
	t.end();
});
